...
export default function AddExpense() {
  ...
  const { aliases } = useAddressAliases();
  const { toast } = useToast();

  // Convert aliases array to lookup map
  const aliasMap = aliases.reduce((acc, alias) => {
    acc[alias.name.toLowerCase()] = alias.address;
    return acc;
  ...
  const fetchSuggestions = async (text: string, setSuggestions: (suggestions: GeoapifyFeature[]) => void) => {
    ...
    // Check if text matches a saved alias
    const aliasMatch = aliasMap[text.toLowerCase()];
